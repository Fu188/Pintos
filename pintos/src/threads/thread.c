...
bool thread_mlfqs;
fixed_t load_avg;               /* Record the load_avg of the whole os */
...
/* Sets the current thread's priority to NEW_PRIORITY. 
   If NEW_PRIORITY < the priority of ready_list's head, then call thread_yield() */
void
...
void thread_update_priority(struct thread *t){
  ...
  if(thread_mlfqs){
    ...
    if(!list_empty(&t->lock_holding)){
      ...
      int front_priority = list_entry (list_front (&t->lock_holding), struct lock, elem)->priority;
...
void thread_update_load_avg(void){
  load_avg = FP_ADD(FP_DIV_MIX(FP_MULT_MIX(load_avg, 59),60),FP_DIV_MIX(FP_CONST(thread_ready_count(thread_current())),60));
}
...
void thread_update_priority_mlfqs(struct thread *t){
  if (t ==  idle_thread) return;
  t->priority = FP_ROUND(FP_SUB(FP_SUB(FP_CONST(PRI_MAX),FP_DIV_MIX(t->recent_cpu,4)),FP_MULT_MIX(FP_CONST(2),t->nice)));
